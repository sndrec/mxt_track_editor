...
def _fcurve_to_points(fcu):
    ...
    for kp in fcu.keyframe_points:
        t = kp.co.x / 100.0
        dt_l = (kp.handle_left.x - kp.co.x) / 100.0
        dt_r = (kp.handle_right.x - kp.co.x) / 100.0
        tan_l = ((kp.handle_left.y - kp.co.y) / dt_l / 3.0) if dt_l != 0 else 0.0
        tan_r = ((kp.handle_right.y - kp.co.y) / dt_r / 3.0) if dt_r != 0 else 0.0
...
def _quaternion_matrix_points(fc_quat):
    ...
    for idx, kp in enumerate(w.keyframe_points):
        ...
        q = Quaternion((w.evaluate(frame), x.evaluate(frame), y.evaluate(frame), z.evaluate(frame))).normalized()
        mat = q.to_matrix()
        vals = [mat[0][0], mat[0][1], mat[0][2],
                mat[1][0], mat[1][1], mat[1][2],
                mat[2][0], mat[2][1], mat[2][2]]

        ql = Quaternion((w.evaluate(kp.handle_left.x), x.evaluate(kp.handle_left.x),
                         y.evaluate(kp.handle_left.x), z.evaluate(kp.handle_left.x))).normalized()
        ml = ql.to_matrix()
        vl = [ml[0][0], ml[0][1], ml[0][2],
              ml[1][0], ml[1][1], ml[1][2],
              ml[2][0], ml[2][1], ml[2][2]]

        qr = Quaternion((w.evaluate(kp.handle_right.x), x.evaluate(kp.handle_right.x),
                         y.evaluate(kp.handle_right.x), z.evaluate(kp.handle_right.x))).normalized()
        mr = qr.to_matrix()
        vr = [mr[0][0], mr[0][1], mr[0][2],
              mr[1][0], mr[1][1], mr[1][2],
              mr[2][0], mr[2][1], mr[2][2]]

        dt_l = (kp.handle_left.x - frame) / 100.0
        dt_r = (kp.handle_right.x - frame) / 100.0

        for i in range(9):
            tan_l = ((vl[i] - vals[i]) / dt_l / 3.0) if dt_l != 0 else 0.0
            tan_r = ((vr[i] - vals[i]) / dt_r / 3.0) if dt_r != 0 else 0.0
...
def _export_stage(context, filepath):
    ...
    with open(filepath, 'wb') as f:
        ...
        for idx, (seg, cp) in enumerate(cp_list):
            ...
            data += struct.pack('<f', cp.pos_start[1])
            data += struct.pack('<f', cp.pos_end[1])
